 ...
 * Author: nghia
 *
 * Created on December 5, 2020, 8:21 PM
 */
...
int bubbleSortIntWords(char gift[MAXGIFTS][MAXSTRS][MAXNAMELEN], double details[MAXGIFTS][MAXNUMS], int row);
int main()
{
    ...
    //bubbleSortIntWords(gift,details,row);
...
void toyFormat(char gift[MAXGIFTS][MAXSTRS][MAXNAMELEN], double details[MAXGIFTS][MAXNUMS], int row)
{
    ...
    {   
        ...
        printf("%12s %2.0lf  $%6.2lf  $%5.2lf  $%6.2lf   =  $%8.2lf for  %s\n",
            gift[p][CAT], details[p][QUANT], details[p][PRICE], details[p][TAX], details[p][SHIP], details[p][TOTAL], gift[p][ITEM]);
    }
}
...
int bubbleSortIntWords(char gift[MAXGIFTS][MAXSTRS][MAXNAMELEN], double details[MAXGIFTS][MAXNUMS], int row)
{
    ...
    for (int outer = 0; outer < (row - 1); outer++)
    {
        ...
        {
            if(details[i][TOTAL] > details[i+1][TOTAL])
            {
