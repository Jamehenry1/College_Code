...
class Table:
    def __init__(self, id):
        self.id = id
        self.graph = []
        self.neighbor = [[0,INF],[1,INF],[2,INF],[3,INF],[4,INF],[5,INF]]
...
class NodeT:
    def __init__(self,table):
        self.node = [Table(0),Table(1),Table(2),Table(3),Table(4),Table(5)]
        self.table = table

    def UTable(self,row,col,val):
        self.table[row][col] = val

DVtable = [[0,INF,INF,INF,INF,INF],
...
def PrintTable():
    ...
    z = 0
    print(' DV | {:2} | {:^2} | {:>2} | {:<2} | {:<2} | {:<2} |'.format(0,1,2,3,4,5))
    print('-----------------------------------')
    for row in Dv.table:
        print('| ' + str(z) + ' | {:2} | {:^2} | {:>2} | {:<2} | {:<2} | {:<2} |'.format(*row))
        z = z+1
...
#The DVmaker() job is to get the min value between all the neighbors and send it to sendDV()
def DVmaker(check): #Check is the slot it is calculating
    
    zero = [[0,INF],[1,INF],[2,INF],[3,INF],[4,INF],[5,INF]]
    one = [[0,INF],[1,INF],[2,INF],[3,INF],[4,INF],[5,INF]]
    two = [[0,INF],[1,INF],[2,INF],[3,INF],[4,INF],[5,INF]]
    three = [[0,INF],[1,INF],[2,INF],[3,INF],[4,INF],[5,INF]]
    four = [[0,INF],[1,INF],[2,INF],[3,INF],[4,INF],[5,INF]]
    five = [[0,INF],[1,INF],[2,INF],[3,INF],[4,INF],[5,INF]]
    minValNeigbor = [[0,INF],[1,INF],[2,INF],[3,INF],[4,INF],[5,INF]]
        ...
        if Dv.node[check].neighbor[i][1] != 16: # This gets the neighbor ---- 
            if Dv.node[check].neighbor[i][1] != 0:
                ...
                for j in range(6):
                    ...
                    if i == 5:
                        five[j][1] = Dv.node[check].neighbor[i][1] + Dv.node[i].neighbor[j][1]

                for k in range(6):  # This gets the min value of all the neighbor
                    minValNeigbor[k][1] = min(zero[k][1],one[k][1],two[k][1],three[k][1],four[k][1],five[k][1])
                    
    return minValNeigbor
'''
sendDV() will compare the minvalue that it got from the DVmaker() and then will compare it again
to the original neighbor's DV and if it is less then the original then it will update it 
'''
def sendDV():
    #time = 1
    #count = 1
    ...
    count = 0
    #print("Time: " + str(time) + "-----------------------------")
    DVArray =  [[0,INF],[1,INF],[2,INF],[3,INF],[4,INF],[5,INF]]
    PrintOutputString = []
    
    #This is get the DV of all the neighbors before updating 
    DVArray[0] = DVmaker(0)
    DVArray[1] = DVmaker(1)
    DVArray[2] = DVmaker(2)
    DVArray[3] = DVmaker(3)
    ...
    #After getting all of the new DV from above it will update here
    if printFlag:
        print("\nDV Table: ")
    for i in range(6): #This will check if the new calculated Dv for each neighbor is less then before
        for j in range(6):
...
# This checks for the stable state
def sstate(): 
    if(timeFlag):
        startTime = time.time()
    ...
    if(timeFlag):
        endTime = time.time()
        print("Time until stable: %.3f s" % float(endTime - startTime))
...
def Run():
    ...
    while True:
        ...
        if(input("Would you like to run until stable state? (y) or (n)") == "y"):
            timeFlag = True
        elif(input("Would you like the Print? (y) or (n)") == "y"):
            printFlag = True
    
        num_iter = sstate()
        print("Number of iterations needed: ", num_iter)
